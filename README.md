...
## AI usage
The idea and design of this project is my own and stems from the fact that I needed to test my APIs, didn't know enough about tools like Postman and wanted to learn. What better way is there than to write Go code, right? AI has been used throughout the project to bounce ideas. For some parts I have relied on AI as a sounding board more than others , such as the [code generation and reflection parts](cmd/e2e/load.go). Only recently (in fact today as of the date of this commit) have I actually let AI generate any code. But all code has been reviewed line by line and fixed and refined to my own taste. The reason is that I need to practice working with AI because my workplace now forces me, sigh...
