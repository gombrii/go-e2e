...
## AI usage
The idea and design of this project is my own from the start and stems from the fact that I needed to test my APIs, didn't know enough about tools like Postman and wanted to learn. What better way is there than to write Go code, right? AI has been used throughout the project to bounce ideas ChatGPT in browser style. For some parts I have relied more on AI as a sounding board than others, such as the [code generation and reflection parts](cmd/e2e/load.go). Only recently have I actually let AI generate _any_ code, using Claude Code. I have kind of had to do this to to familiarize myself with it as my workplace now forces us to work this way. But in this project I have never done the whole automated AI-workflow with skills or whatnot. Even when I have let AI write code it's been a back and forth between the AI and me, it writes some parts, I write some parts or rewrite parts of what it wrote. What it genuinely helps me with is documentation and finding pitfalls. I am still involved in every if-statement and variable assignment. This is a little passion project of mine and I want to be the writer of it to the largest extent possible, not just hand it off to some hoard of agents. But any commits of which AI has been a part does have Claude as a co-writer.
